...
extension FixedWidthInteger {
  /// Funnel shift with rounding, trapping on overflow
  @inlinable @inline(__always)
  public init(
    shifting bits: (high: Self, low: Magnitude),
    rightBy count: Int,
    rounding rule: RoundingRule = .down
  ) {
    var overflow: Bool = false
    self.init(
      shifting: (bits.high, bits.low),
      rightBy: count,
      rounding: rule,
      overflow: &overflow
    )
  ...
  /// Funnel shift with rounding
  @inlinable @inline(__always)
  static func shiftingReportingOverflow(
    _ bits: (high: Self, low: Magnitude),
    rightBy count: Int,
    rounding rule: RoundingRule = .down
  ) -> (result: Self, overflow: Bool) {
    var overflow: Bool = false
    if count <= 0 {
      // This is really a left shift; no rounding and we only need low bits,
      // but we do need to check for overflow.
      if count <= -Self.bitWidth {
        // Left shift at least as large as word size. Result is
        // unconditionally zero, and overflow is set if either high or
        // low was non-zero.
        return (
          result: 0,
          overflow: (bits.high | Self(truncatingIfNeeded: bits.low)) != 0
        )
      } else {
        // Left shift smaller than word size. Result comes only from the low
        // bits, overflow is set if any non-sign bits are shifted off the top,
        // or if high is not the sign extension of low.
        let signedLow = Self(truncatingIfNeeded: bits.low)
        let sign = signedLow.signbit
      }
    } else if count < Self.bitWidth {
      // Bits from both high and low contribute to the result.
      // Construct a mask covering the bits to be shifted off.
      let mask = Magnitude(1) &<< count &- 1
      let hcnt = Self.bitWidth - count
      /*
      let lost = Magnitude(truncatingIfNeeded: bits.low) & mask
      let fill = bits.high &<< (Self.bitWidth - count)
      let floor = fill | Self(truncatingIfNeeded: bits.low &>> count)
       */
      switch rule {
      case .down:
        self = bits.high &<<
      case .up:
        let (_, carry) = lost.addingReportingOverflow(.max)
        self = floor + (carry ? 1 : 0)
      case .towardZero:
        let onesIfNegative = Magnitude(truncatingIfNeeded: bits.high.signbit)
        let (_, carry) = lost.addingReportingOverflow(onesIfNegative)
        self = floor + (carry ? 1 : 0)
      case .awayFromZero:
        let onesIfPositive = ~Magnitude(truncatingIfNeeded: bits.high.signbit)
        let (_, carry) = lost.addingReportingOverflow(onesIfPositive)
        self = floor + (carry ? 1 : 0)
      case .toOdd:
        let (_, carry) = lost.addingReportingOverflow(.max)
        self = floor | (carry ? 1 : 0)
